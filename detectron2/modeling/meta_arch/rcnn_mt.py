...
__all__ = ["DAobjTwoStagePseudoLabGeneralizedRCNN", "ProposalNetwork"]
...
class DAobjTwoStagePseudoLabGeneralizedRCNN(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        return {
            # "offline_backbone": offline_boffline_backboneackbone,
            "backbone": backbone,
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], clipcap_model=None, branch='supervised'):
        ...
        if branch == "supervised":
            ...
            if self.proposal_generator is not None:
                ...
                assert "proposals" in batched_inputs[0]
                proposals_rpn = [x["proposals"].to(self.device) for x in batched_inputs]
                proposal_losses = {}

            if self.use_clip_c4:  # use C4 + resnet weights from CLIP
                if self.use_clip_attpool:  # use att_pool from CLIP to match dimension
                    _, detector_losses = self.roi_heads(images, features, proposals_rpn, gt_instances,
                                                        res5=self.backbone.layer4, attnpool=self.backbone.attnpool,
                                                        compute_loss=True, branch=branch)
                else:  # use default mean pool
                    _, detector_losses = self.roi_heads(images, features, proposals_rpn, gt_instances,
                                                        res5=self.backbone.layer4, compute_loss=True, branch=branch)
            else:  # default setting
                _, detector_losses = self.roi_heads(images, features, proposals_rpn, gt_instances, compute_loss=True,
                                                    branch=branch)
            ...
            if self.use_clip_c4:  # use C4 + resnet weights from CLIP
                if self.use_clip_attpool:  # use att_pool from CLIP to match dimension
                    proposals_roih, ROI_predictions = self.roi_heads(images, features, proposals_rpn, targets=None,
                                                                     res5=self.backbone.layer4,
                                                                     attnpool=self.backbone.attnpool,
                                                                     compute_loss=False, branch=branch)
                else:  # use default mean pool
                    proposals_roih, ROI_predictions = self.roi_heads(images, features, proposals_rpn, targets=None,
                                                                     res5=self.backbone.layer4, compute_loss=False,
                                                                     branch=branch)
            else:  # default setting
                proposals_roih, ROI_predictions = self.roi_heads(images, features, proposals_rpn, targets=None,
                                                                 compute_loss=False,
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return DAobjTwoStagePseudoLabGeneralizedRCNN._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
